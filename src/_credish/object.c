...
static void zset_val_free(void *value) { bufpool_free(value, sizeof(double)); }
static void *zset_val_dup(void *value)
{
...
credishObject *obj_create_zset(void)
{
    ...
    zset *zset_ptr = bufpool_alloc(sizeof(zset));
    if (!zset_ptr)
    {
    ...
    zset_ptr->dict = dict_create(&zset_dict_type);
    zset_ptr->zsl = zsl_create();
    obj->type = OBJ_ZSET;
    obj->encoding = OBJ_ENCODING_RAW;
    obj->ptr = zset_ptr;
    return obj;
...
void obj_free(credishObject *obj)
{
    ...
    case OBJ_ZSET:
    {
        zset *zset_ptr = (zset *)obj->ptr;
        dict_free(zset_ptr->dict);
        zsl_free(zset_ptr->zsl);
        bufpool_free(zset_ptr, sizeof(zset));
        break;
    }
    }
    bufpool_free(obj, sizeof(*obj));
...
char *obj_string_ptr(const credishObject *obj, int *len_out)
{
    if (!obj || obj->type != OBJ_STRING)
        return NULL;
    sds sds_str = (sds)obj->ptr;
    if (len_out)
        *len_out = (int)SDS_LEN(sds_str);
    return sds_str;
}
