...
#define SDS_PREALLOC_THRESHOLD (1024 * 1024) /* 1 MB: switch from 2x to linear growth */
#define SDS_MAX_ALLOC (512 * 1024 * 1024)    /* 512 MB: absolute hard limit */
...
sds sds_catlen(sds dest, const char *data, size_t len);
sds sds_catlenprintf(sds dest, const char *fmt, ...);
