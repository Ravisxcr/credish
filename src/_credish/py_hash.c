...
{
    Py_ssize_t mapping_size = PyDict_Size(mapping);
    *n_out = mapping_size;
    if (mapping_size == 0)
    {
        *fields_out = NULL;
    ...
    sds *fields = calloc((size_t)mapping_size, sizeof(sds));
    sds *values = calloc((size_t)mapping_size, sizeof(sds));
    if (!fields || !values)
    {
        free(fields);
        free(values);
                             ...
                             sds *fields, sds *values, Py_ssize_t n, int *added_out)
{
    ...
    {
        aof_argv[0] = key;
        aof_lens[0] = (size_t)keylen;
        for (Py_ssize_t i = 0; i < n; i++)
        {
            aof_argv[1 + 2 * i] = fields[i];
            aof_lens[1 + 2 * i] = SDS_LEN(fields[i]);
            aof_argv[2 + 2 * i] = values[i];
            aof_lens[2 + 2 * i] = SDS_LEN(values[i]);
        }
...
PyObject *py_hset(PyObject *self, PyObject *args, PyObject *kwargs)
{
    (void)self;
    ...
    if (has_mapping)
    {
        if (mapping_to_sds_pairs(mapping, &fields, &values, &n) != 0)
            return NULL;
        if (n == 0)
...
PyObject *py_hmset(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OOO", &handle, &key_obj, &mapping))
        return NULL;
...
PyObject *py_hdel(PyObject *self, PyObject *args)
{
    (void)self;
    PyObject *handle, *key_obj, *fields_list;
    if (!PyArg_ParseTuple(args, "OOO", &handle, &key_obj, &fields_list))
        return NULL;
    if (!PyList_Check(fields_list))
    {
        PyErr_SetString(PyExc_TypeError, "expected list");
...
PyObject *py_hexists(PyObject *self, PyObject *args)
{
    (void)self;
    PyObject *handle, *key_obj, *field_obj;
    if (!PyArg_ParseTuple(args, "OOO", &handle, &key_obj, &field_obj))
        return NULL;
    credish_store *store = credish_get_store(handle);
    if (!store)
        return NULL;
...
PyObject *py_hgetall(PyObject *self, PyObject *args, PyObject *kw)
{
    ...
    if (o)
    {
        ...
        dict_iterator *it = dict_iter_new((dict *)o->ptr);
        if (it)
        {
            dict_entry *e;
...
PyObject *py_hlen(PyObject *self, PyObject *args)
{
    (void)self;
    PyObject *handle, *key_obj;
    if (!PyArg_ParseTuple(args, "OO", &handle, &key_obj))
        return NULL;
    credish_store *store = credish_get_store(handle);
    if (!store)
        return NULL;
...
PyObject *py_hincrby(PyObject *self, PyObject *args)
{
    ...
    PyObject *handle, *key_obj, *field_obj;
    long long amount;
    if (!PyArg_ParseTuple(args, "OOOL", &handle, &key_obj, &field_obj, &amount))
        return NULL;
    credish_store *store = credish_get_store(handle);
    if (!store)
        return NULL;
